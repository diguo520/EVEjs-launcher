...
const api = {
  ...
  configRepairClientDisplay: () => ipcRenderer.invoke("config:repairClientDisplay"),
  ...
  modsTemplates: () => ipcRenderer.invoke("mods:templates"),
  modsSubmitPrepare: (input: Record<string, unknown>) => ipcRenderer.invoke("mods:submitPrepare", input),
  modsSubmitGithub: (id: string, version: string) => ipcRenderer.invoke("mods:submitGithub", id, version),
  modsMySubmissions: () => ipcRenderer.invoke("mods:mySubmissions"),
  modsMyMods: () => ipcRenderer.invoke("mods:myMods"),
  modsMarketList: (force?: boolean) => ipcRenderer.invoke("mods:marketList", !!force),
  modsOpenModFolder: (folder: string) => ipcRenderer.invoke("mods:openModFolder", folder),
  modsReadme: (folder: string) => ipcRenderer.invoke("mods:readme", folder),
  modsMarketInstall: (entry: Record<string, unknown>) => ipcRenderer.invoke("mods:marketInstall", entry),
  modsSaveText: (defaultName: string, content: string) => ipcRenderer.invoke("mods:saveText", defaultName, content),
  onModDownloadProgress: (cb: (p: { id: string; downloaded: number; total?: number; percent?: number; mirror?: string }) => void) => {
    const h = (_e: IpcRendererEvent, p: { id: string; downloaded: number; total?: number; percent?: number; mirror?: string }) => cb(p);
  ...
  modsGithubTokenStatus: () => ipcRenderer.invoke("mods:githubTokenStatus"),
  modsGithubTokenSave: (token: string) => ipcRenderer.invoke("mods:githubTokenSave", token),
  modsGithubTokenClear: () => ipcRenderer.invoke("mods:githubTokenClear"),
  modsGithubTokenCheck: (token?: string) => ipcRenderer.invoke("mods:githubTokenCheck", token),
  modsRevealSubmissionZip: (zipPath: string) => ipcRenderer.invoke("mods:revealSubmissionZip", zipPath),
  modsCreate: (draft: Record<string, unknown>) => ipcRenderer.invoke("mods:create", draft),
  authorGet: () => ipcRenderer.invoke("author:get"),
  authorSetName: (name: string) => ipcRenderer.invoke("author:setName", name),
  authorExportKey: () => ipcRenderer.invoke("author:exportKey"),
  authorImportKey: () => ipcRenderer.invoke("author:importKey"),
  authorOpenKeyFolder: () => ipcRenderer.invoke("author:openKeyFolder"),
